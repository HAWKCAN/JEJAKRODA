...
@section('title', 'Edit Kendaraan — Aspal Seru')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100">
            <h2 class="text-sm font-bold text-[#162740]">Edit: {{ $vehicle->name }}</h2>
            <p class="text-xs text-slate-400 mt-0.5">Perbarui data kendaraan kamu.</p>
        </div>

        <form method="POST" action="{{ route('manager.vehicles.update', $vehicle->id) }}"
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="type" value="motor" class="sr-only peer"
                               {{ old('type', $vehicle->type) === 'motor' ? 'checked' : '' }}>
                    ...
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="type" value="mobil" class="sr-only peer"
                               {{ old('type', $vehicle->type) === 'mobil' ? 'checked' : '' }}>
                        <div class="flex items-center justify-center gap-2 py-3 rounded-xl border-2
                                    text-sm font-semibold transition-all
                                    border-slate-200 text-slate-500 bg-slate-50
                                    peer-checked:border-sky-500 peer-checked:text-sky-600 peer-checked:bg-sky-50">
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1.5">
                    Foto Kendaraan
                    <span class="text-slate-400">(kosongkan jika tidak ingin mengganti)</span>
                </label>

                {{-- Foto saat ini --}}
                @if($vehicle->image_url)
                <div class="mb-2">
                    <p class="text-[11px] text-slate-400 mb-1">Foto saat ini:</p>
                    <img src="{{ Storage::url($vehicle->image_url) }}"
                         alt="{{ $vehicle->name }}"
                ...
                <div id="drop-zone"
                     class="relative border-2 border-dashed border-slate-200 rounded-xl
                            bg-slate-50 hover:border-sky-400 transition-colors
                            text-center cursor-pointer overflow-hidden">
                    <input type="file" name="image" id="image-input" accept="image/*"
                           class="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10"
                           onchange="previewImage(event)">
                    <div id="upload-placeholder" class="py-6 px-4">
                        <div class="text-2xl mb-1">📷</div>
                        <p class="text-sm font-semibold text-slate-500">Klik untuk ganti foto</p>
                        <p class="text-xs text-slate-400 mt-0.5">JPG, PNG, WebP — maks. 3 MB</p>
                    </div>
